...
//const int sensors_per_slave = {0,1,2,4};
...
// Angle limits for each JOINT // ask about these limits
...
void get_encoder_values() {
    ...
    for(int slave_addr = 1; slave_addr <= 3; slave_addr++) {
        Wire.requestFrom(slave_addr, sensors_per_slave[slave_addr]); // request 6 bytes from slave
        int i = 0;
        byte read_bytes[sensors_per_slave[slave_addr]];//change 4 bytes
        ...
        for (int i = 0; i < sensors_per_slave[slave_addr - 1]; i++) { // for each sensor attatched to the slave
            ...
            sens_val = read_bytes[4 * i] >> 24| (read_bytes[4 * i + 1] >> 16) | (read_bytes[4*i+2] >> 8)| (read_bytes[4*i+3]); // convert bytes to int //needs 4 bytes
...
void receiveEvent(int number_of_bytes) {
    if(Wire.available()){
        ...
        while (Wire.available()) {
            ...
            actual_pos[sensor_no] = (actual_pos[sensor_no] << 24)| b;
