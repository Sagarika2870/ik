...
def positionalIK(): # perform inverse kinematics, move the arm to the new position
    ...
    try:
        ...
        # MOVE THE ARM TO THE NEW PLACE!!!!!!!!!!
        sendAngles = [ savedJointAngles[0], savedJointAngles[1], savedJointAWWngles[2], savedJointAngles[3], savedJointAngles[4], savedJointAngles[5], savedGripperAngle ]
        sendAngleValues(sendAngles)
...
def main():
    ...
    storageFile = open('/home/osboxes/rsx/rover/src/ArmControl/savedJointAngles.txt', 'w')
    storageFile.write( str(savedJointAngles) )
    storageFile.close()
    storageFile = open('/home/osboxes/rsx/src/rover/src/ArmControl/savedGripperAngle.txt', 'w')
